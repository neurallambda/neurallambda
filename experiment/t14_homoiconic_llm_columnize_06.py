...
Add uncausal masking for metatokens
...
def columnwise_collate_fn(
    ...
    for item in batch:
        padded_item = item + [{'type': 'pad_block',
                               ...
                               **{k: (None, None) for k in lor_ix_keys}  # TODO: ix parse keys have wrong representation (should be list, num_layers long)
                               }] * (max_cols - len(item))
...
def create_column_batch_inputs(
    ...
    """
    col_batch = columnwise_collate_fn(batch, lor_ix_keys)  # list of columns with multiple rows (dictionaries of raw data, eg text)
    col_batch_inputs = []
    B = len(col_batch[0])  # nrows of first column
    max_positions = torch.tensor([0] * B, device=device)

    for column in col_batch:  # iterate over each column of raw data

        texts = [sample['content'] for sample in column]
        include_in_losses = [sample['include_in_loss'] for sample in column]
        ...
        # lor parsing ixs
        layer_batch_left_lor_ixs  = torch.zeros(num_layers, B, dtype=torch.long) - 1  # -1 means no parse
        layer_batch_right_lor_ixs = torch.zeros(num_layers, B, dtype=torch.long) - 1

        batch_all_parse_ixs = [[]] * B  # used for uncausal_mask

        for k in lor_ix_keys:  # eg ['lor_qs', 'lor_ks', 'lor_vs', ...]
            for batch_ix, sample in enumerate(column):  # iter each data sample
                all_parse_ixs = []
                per_layer_parse_ixs = sample[k]  # eg: [None, None, (1, 2), None, None, ...n_layers]
                for layer_ix, parse_ixs in enumerate(per_layer_parse_ixs):
                    if parse_ixs is None:
                        continue
                    p1 = parse_ixs[0]
                    p2 = parse_ixs[1]
                    layer_batch_left_lor_ixs[layer_ix][batch_ix]  = p1
                    layer_batch_right_lor_ixs[layer_ix][batch_ix] = p2
                    all_parse_ixs.extend([p1, p2])
                batch_all_parse_ixs[batch_ix] = batch_all_parse_ixs[batch_ix] + all_parse_ixs  # accumulate all parse ixs across all k into appropriate batch ix
            inputs[k] = (layer_batch_left_lor_ixs, layer_batch_right_lor_ixs)

        uncausal_mask = torch.zeros(B, S, dtype=torch.bool)
        for batch_ix, ixs in enumerate(batch_all_parse_ixs):
            for ix in ixs:
                uncausal_mask[batch_ix, ix] = 1

        inputs['uncausal_mask'] = uncausal_mask
...
def create_dataloader(
    ...
    return DataLoader(
        ...
        collate_fn=lambda batch: create_column_batch_inputs(batch, num_layers, lor_ix_keys, tokenizer, use_uncausal_metatokens, device)
    )
