...
RESULTS: LoRW params are pre-baked (not generated by LLM) and this trains em. On to letting the LLM generate the LoRWs!
...
past_key_values across generations. '''
    ...
    return torch.cat(out_logits, dim=-2), attention_mask, past_key_values


def generate(model, input_ids, attention_mask, lors, max_new_tokens, past_key_values=None):
...
def loss_fn(batch_cols, out_logits):
    last_col = batch_cols[-1]['input_ids']
    last_val = last_col[:, -1]

    # NOTE: we use the penultimate output, which should predict "lion" in this
    # example (ie we don't want the word *after* lion, which doesn't exist in
    # the dataset), because given the prefix, I want it to eagerly predict lion
    # kind of stuff.
    penultimate_output = out_logits[:, -2]
    return F.cross_entropy(penultimate_output, last_val)

def run_epoch(model, lors, dataloader, loss_fn, optimizer, device, train=True):
    ...
    with torch.set_grad_enabled(train):
        for batch_cols in tqdm(dataloader, desc="Training" if train else "Evaluating"):
            ...
            loss = loss_fn(batch_cols, out_logits)
...
prompt_chonkss = [
    ["Once upon a time in a galaxy far away, ", "there was a^@Q lion"],
    ["Once upon a time in a galaxy far away, ", "there was a^@Q lion"],
]
...
if True:
    ...
    for epoch in range(num_epochs):
        train_loss = run_epoch(model, lors, train_dl, loss_fn, optimizer, DEVICE, train=True)
        print(f"Epoch {epoch+1}/{num_epochs}, Train Loss: {train_loss:.8f}")

    # col_output_logits, attention_mask, past_key_values = forward_columns(model, col_inputs, lors)

    # col_output_ids = col_output_logits.argmax(dim=-1)
...
with torch.no_grad():
    scale = 0.8
    slors = scale_lors(lors, model.config.num_hidden_layers, scale)

    test_prompts = [
        "Once upon a time in a galaxy far away, there was a^@Q ",
        "Hi, ",
        # "Once upon a time in a galaxy far away, there was a^@Q",
    ]
    ...
    output_ids, col_attention_mask, col_past_key_values = generate(model, input_ids, attention_mask, slors, max_new_tokens=20, past_key_values=None)
